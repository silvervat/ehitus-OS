...
export const Timeline: React.FC<TimelineProps> = ({ data, startDateKey = 'start_date', endDateKey = 'end_date', labelKey = 'name' }) => {
    ...
    const getTaskStyle = (task: TimelineTask) => {
        ...
        const effectiveWidth = Math.max(20, duration * zoom.pixelsPerDay);
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 overflow-hidden relative select-none">
            ...
            <div className="flex-1 flex overflow-hidden relative">
                <div style={{ width: SIDEBAR_WIDTH }} className="flex-shrink-0 bg-white border-r border-slate-200 flex flex-col z-20 shadow-lg">
                    ...
                    <div ref={sidebarRef} className="flex-1 overflow-hidden bg-white">
                         <div style={{ paddingTop: 0 }}>
                             {tasks.map((task) => (
                                 <div key={task.id} style={{ height: ROW_HEIGHT }} className="border-b border-slate-100 flex items-center px-4 hover:bg-slate-50 group transition-colors">
                ...
                <div ref={gridRef} className="flex-1 overflow-auto bg-slate-50 custom-scrollbar relative" onScroll={handleGridScroll}>
                     <div style={{ height: HEADER_HEIGHT, minWidth: totalGridWidth }} className="sticky top-0 bg-white z-10 shadow-sm flex flex-col border-b border-slate-200">
                         <div className="h-1/2 flex border-b border-slate-100">
                             {calendarDates.map((date, i) => {
                                 const isFirst = date.getDate() === 1 || i === 0;
                                 if (isFirst) {
                                     return (
                                         <div key={i} className="px-2 py-1 text-xs font-bold text-slate-600 border-r border-slate-100 bg-slate-50" style={{ width: zoom.pixelsPerDay * (30 - date.getDate()) }}>
                         ...
                         <div className="h-1/2 flex">
                            {calendarDates.map((date, i) => {
                                 ...
                                 return (
                                     <div key={i} style={{ width: zoom.pixelsPerDay }} className={`flex flex-col items-center justify-center border-r border-slate-100 text-[10px] ${isWeekend ? 'bg-slate-50 text-red-300' : 'text-slate-500'}`}>
                     ...
                     <div style={{ minWidth: totalGridWidth, height: tasks.length * ROW_HEIGHT, position: 'relative' }}>
                         <div className="absolute inset-0 flex pointer-events-none">
                            {calendarDates.map((date, i) => {
                                 const isWeekend = date.getDay() === 0 || date.getDay() === 6;
                                 return (
                                     <div key={i} style={{ width: zoom.pixelsPerDay }} className={`border-r border-slate-100 h-full ${isWeekend ? 'bg-slate-50/50' : ''}`} />
                                 )
                             })}
                         </div>
                         ...
                         {tasks.map((task, i) => {
                             ...
                             return (
                                 <div key={task.id} className="absolute w-full border-b border-slate-100/50 flex items-center group" style={{ height: ROW_HEIGHT, top: i * ROW_HEIGHT }}>
                                     <div 
                                     ...
                                     >
                                         ...
                                         <div className="absolute left-0 top-0 bottom-0 w-2 cursor-w-resize hover:bg-black/20" onMouseDown={(e) => handleMouseDown(e, task, 'resize-left')}></div>
                                         <div className="absolute right-0 top-0 bottom-0 w-2 cursor-e-resize hover:bg-black/20" onMouseDown={(e) => handleMouseDown(e, task, 'resize-right')}></div>
                                     </div>
                                 </div>
                             )
                         })}
